...
public class BattleSystem : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        otherUnit.currentHP = otherUnit.maxHP;
    ...
    private void OpenSkillMenu()
    {
        ...
        {
            //플레이어 유닛(포켓몬)이 가진 스킬 수 만큼 개방하는 기능
            if (i < playerUnit.skills.Length)
            {
                skill_buttons[i].gameObject.SetActive(true);
                skill_buttons[i].GetComponentInChildren<Text>().text = playerUnit.skills[i].skillName;
            }
            //없으면 false
            else
            {
