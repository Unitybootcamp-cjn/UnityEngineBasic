...
public class Attack : MonoBehaviour
{
    ...
    private void Update()
    {
        //hit가 활성화된 상태라면 return해서 hit에 대한 처리가 다 끝난 뒤, 새롭게 hit가 false가 되면 작동할 수 있게 설정함
        if (hit) return;
        ...
        if(Vector3.Distance(transform.position, target_pos) <= 0.1f)
        {
            ...
            if (target != null)
            {
                ...
                //유닛이 가진 체력을 데미지만큼 감소시킨다.
                target.GetComponent<Unit>().HP -= damage;
                //감소하면 플레이어나 몬스터가 데미지 처리하는 함수가 처리되도록 설정해줄 필요가 있음.
                ...
                //파티클의 duration은 현재 main.duration으로 접근 가능함.
                //일반적으로 코루틴은 Start 영역에서 진행하나, Update여도 특정 조건일 때만 실행되는 수준에서는 사용하는 경우도 있다.
            }
        }
    }
