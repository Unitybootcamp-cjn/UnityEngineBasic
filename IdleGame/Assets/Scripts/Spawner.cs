...
public class Spawner : MonoBehaviour
{
    ...
    IEnumerator CSpawn()
    {
        ...
        {
            // 3. 어떤 형태로 생성할 것 인가?
            pos = Vector3.zero + Random.insideUnitSphere * 20.0f; // Random.insideUnitSphere : 1f 의 반지름 내의 구를 만들어서 그 안에서 랜덤하게.
            // 생성 높이 통일(y 좌표 = 0)
            pos.y = 0.0f;

            // 생성된 거리를 기준으로 재 생성 유도
            while(Vector3.Distance(pos, Vector3.zero) <= 10.0f)
            {
                pos = Vector3.zero + Random.insideUnitSphere * 20.0f;
                pos.y = 0.0f;
            }
